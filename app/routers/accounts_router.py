...
def create_account(user_create: AccountCreate, session: Session = Depends(get_session), active_user: dict = Depends(get_user_with_role)):
    ...
    existing_user = get_authenticated_user_or_404(session, active_user)

    #---Checking if the user already has this account type---
    validate_no_duplicate_account_type(session, existing_user, user_create.account_type)
...
def get_user_account(session: Session= Depends(get_session), active_user: dict= Depends(get_user_with_role)):
...
def get_specific_account(account_id: int, session: Session= Depends(get_session), active_user: dict= Depends(get_user_with_role)):
...
#---Getting the logged-in user email using the access token---
    ...
    account = get_account_by_id_or_404(session, account_id, user, active_user)
...
def update_user_account(account_id: int, user_update: AccountUpdate, session: Session= Depends(get_session), active_user: dict= Depends(get_user_with_role)):
    ...
    account = apply_account_update(session, account, user_update, account_id)
...
def close_user_account(account_id: int, session: Session= Depends(get_session), active_user: dict= Depends(get_user_with_role)):
    ...
    validate_account_closure(account, account_id)

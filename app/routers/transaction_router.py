...
def create_deposit_transactions(transaction_create: TransactionCreate, session: Session= Depends(get_session), active_user: dict= Depends(get_user_with_role)):

    logger.info("Deposit transaction requested.")

    transaction_receipt= deposit_helper_function(session, transaction_create, active_user)
...
def create_transfer(transfer_create: TransferCreate, session: Session= Depends(get_session), active_user: dict= Depends(get_user_with_role)):

    logger.info("Transfer transaction requested.")

    sender_receipt, receiver_receipt = make_transfer(transfer_create, session, active_user)
...
def get_statement_of_account(search: StatementSearch= Depends(), session: Session= Depends(get_session), active_user: dict= Depends(get_user_with_role)):
    
    logger.info("Statement of account requested.")

  #---Getting the authenticated user---
    user = get_authenticated_user_or_404(session, active_user)
...
def get_transaction_history(account_id: int, skip: int= 0, limit: int= Query(default= 10, le= 100), session: Session= Depends(get_session), active_user: dict= Depends(get_user_with_role)):

    logger.info(f"Transaction history requested for account {account_id}.")

    return fetch_transaction_history(account_id, skip, limit, session, active_user )


#---Creating the endpoint that gets one specific transaction---
@router.get("/{ref_id}", response_model= TransactionRead, status_code= 200)
def get_transaction_references(ref_id: str, session: Session= Depends(get_session), active_user: dict= Depends(get_user_with_role)):

    logger.info(f"Transaction lookup requested for reference {ref_id}.")

    return fetch_reference(ref_id, session, active_user)